...
class Customer_Admin_Option_Screen(QDialog):
    def __init__(self):
        ...
        self.movie = QMovie(r"Project_Source_Code/Flight Gif.gif")
...
class OTP_Screen(QDialog):
    ...
    def OTP_Function(self):
        ...
        if len(Customer_Phone_Number) == 0:
            ...
            account_sid = 'AC61d16ee3b9132257b107e39cdfb65a04'
            auth_token = 'ec319bd41e3f372b611694d102e083b3'
...
class Update_Customer_Information(QDialog):
    ...
    def gotoReenterDetails(self):
        ...
        # First, delete from FULL_PROFILE_ACCOUNT
        query1 = "DELETE FROM FULL_PROFILE_ACCOUNT WHERE Customer_Name = %s"
        tpl1 = (Email_Field,)
        cursor.execute(query1, tpl1)

        # Then, delete from initial_info_account
        query2 = "DELETE FROM initial_info_account WHERE Username = %s"
        tpl2 = (Email_Field,)
        cursor.execute(query2, tpl2)

        print("Deleted records for:", Email_Field)
...
class Additional_Info(QDialog):
    def __init__(self):
        ...
        self.Flight_Info_TableWidget.setColumnWidth(2, 150)
        ...
        if result:
            ...
            query = f"SELECT * FROM COMPANY NATURAL JOIN flights WHERE C_Name = %s;"
            tpl = (C_Name,)
            cursor.execute(query, tpl)
            ...
            self.Flight_Info_TableWidget.verticalHeader().setVisible(False)
...
class Cancellation(QDialog):
    def __init__(self):
        ...
        data = [
            {
                "Cancellation ID": self.Cancellation_ID,
                "Cancellation Payment ID": Payment_ID,
                "Cancellation Refund": self.Refund_Cost,
                "Cancellation Date": self.Cancellation_Date,
            }
        ]

        self.Cancellation_Info_TableWidget.setRowCount(len(data))
        ...
        db.commit()
        db.close()
        print("Successfully inserted values in the Cancellation relation")
